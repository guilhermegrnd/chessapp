...
export function updateTakenPieces(piecesTaken:any,pieceTaken:any) {
  let updatedPiecesTaken = [...piecesTaken];
  let alreadyTaken = false;
  if(updatedPiecesTaken.length > 0 ) {
    updatedPiecesTaken = updatedPiecesTaken.map((item) => {
      if(item.class === pieceTaken.class) {
        alreadyTaken = true;
  ...
  if(!alreadyTaken) {
    updatedPiecesTaken.push({ ...pieceTaken, count: 1 })
  }
  return updatedPiecesTaken;
}

export function executePlay(board:any, possibleMoves:any, match:any, pieceSelected:any, square:any, BlackPiecesTaken:any, WhitePiecesTaken:any) {
  ...
  if(!possibleMoves[row][column]) {
    ...
    if(checkTest) {
      ...
      modalShow = true;
      modalMessage = language() === `en_us` ? `You can't put yourself in Check!` : `Você não pode se colocar em Xeque!`;
      onMove = false;
    } else {
      move = true;
    }
  ...
  let updatedWhitePiecesTaken:any = [...WhitePiecesTaken];
  let updatedBlackPiecesTaken:any = [...BlackPiecesTaken];
  
  if(move) {
    if(!emptySquare(destinationRow[column])) {
      let pieceTaken:any = destinationRow[column];
      if(pieceTaken.color === 1) {
        updatedWhitePiecesTaken = updateTakenPieces(updatedWhitePiecesTaken,pieceTaken);
      } else {
        updatedBlackPiecesTaken = updateTakenPieces(updatedBlackPiecesTaken,pieceTaken);
      }
    }
    ...
    if(inCheck(adversaryColor(match.currentPlayer),board,match)) {
      if (inCheckMate(adversaryColor(match.currentPlayer),board,match)) {
        ...
        //GAME OVER
        modalShow = true;
        modalMessage = language() === `en_us` ? `CheckMate!  ${match.currentPlayer === 1 ? 'White' : 'Black'} Won!` : `XequeMate!  ${match.currentPlayer === 1 ? 'Brancas' : 'Pretas'} Venceram!`;
      } else {
        check = true;
        modalShow = true;
        modalMessage = language() === `en_us` ? `Check!` : `Xeque!`;
      }
    } else {
      check = false;
    }
